...
public class LibraryManagementProject {
	...
	public LibraryManagementProject(){
		member_id = 0;
		book_id = 0;
	}
	...
	static void removeLoanByID() {
		if (loans.isEmpty()) {
			...
			Scanner in = new Scanner(System.in);
			// get the user input
			System.out.print("Member ID who was Borrower: ");
			int mid = in.nextInt();
			System.out.print("Book ID number Which was Borrowed: ");
			int bid = in.nextInt();
			for ( int i=0 ; i<loans.size(); i++) {
				if (loans.get(i).member_id == mid && loans.get(i).book_id == bid) {
					loans.remove(i);
					System.out.println("Loan with Member ID: " + mid + " and Books ID " + bid + " Removed");
					f = true;
					break;
				}
			}
